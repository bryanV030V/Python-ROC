...
def validate_letters(char):
    return char.isalpha() or char == " "  # Alleen letters en spaties toestaan

def validate_numbers(char):
    return char.isdigit()  # Alleen cijfers toestaan
...
# Voeg validatie toe voor merk_combobox
merk_combobox['validate'] = 'key'
merk_combobox['validatecommand'] = (root.register(validate_letters), '%S')
...
# Combobox voor maat (S, M, L, XL, etc.)
tk.Label(root, text="Maat").grid(row=2, column=0, sticky="w", padx=5)
maat_combobox = ttk.Combobox(root, values=["S", "M", "L", "XL", "XXL"])
maat_combobox.grid(row=2, column=1, sticky="ew", padx=5)

# Voeg validatie toe voor maat_combobox
maat_combobox['validate'] = 'key'
maat_combobox['validatecommand'] = (root.register(validate_letters), '%S')

# Combobox voor Hoeveelheid
tk.Label(root, text="Hoeveelheid").grid(row=3, column=0, sticky="w", padx=5)
hoeveelheid_combobox = ttk.Combobox(root)
hoeveelheid_combobox.grid(row=3, column=1, sticky="ew", padx=5)

# Voeg validatie toe voor hoeveelheid_combobox
hoeveelheid_combobox['validate'] = 'key'
hoeveelheid_combobox['validatecommand'] = (root.register(validate_numbers), '%S')

# Knop om kledingstuk toe te voegen
voeg_toe_knop = tk.Button(root, text="Voeg Kledingstuk Toe", command=voeg_kledingstuk_toe)
voeg_toe_knop.grid(row=4, column=1, sticky="ew", padx=5)

# Knop om kledingstuk te verwijderen
verwijder_knop = tk.Button(root, text="Verwijder Kledingstuk", command=verwijder_kledingstuk)
verwijder_knop.grid(row=4, column=2, sticky="ew", padx=5)

# Lijstbox voor weergave van kledingstukken
lijstbox = tk.Listbox(root, height=10)
lijstbox.grid(row=5, column=0, columnspan=3, sticky="ew", padx=5, pady=5)

# Filtervelden en knop om de lijst bij te werken
tk.Label(root, text="Filter Merk").grid(row=6, column=0, sticky="w", padx=5)
filter_merk_combobox = ttk.Combobox(root)
filter_merk_combobox.grid(row=6, column=1, sticky="ew", padx=5)

tk.Label(root, text="Filter Type").grid(row=7, column=0, sticky="w", padx=5)
filter_type_combobox = ttk.Combobox(root, values=["Shirt", "Sweater", "Jas", "Broek", "Trui", "Vest"])
filter_type_combobox.grid(row=7, column=1, sticky="ew", padx=5)

tk.Label(root, text="Filter Maat").grid(row=8, column=0, sticky="w", padx=5)
filter_maat_combobox = ttk.Combobox(root, values=["S", "M", "L", "XL", "XXL"])
filter_maat_combobox.grid(row=8, column=1, sticky="ew", padx=5)

tk.Label(root, text="Filter Hoeveelheid").grid(row=9, column=0, sticky="w", padx=5)
filter_hoeveelheid_combobox = ttk.Combobox(root)
filter_hoeveelheid_combobox.grid(row=9, column=1, sticky="ew", padx=5)

# Knop om de lijst te filteren
filter_knop = tk.Button(root, text="Filter Lijst", command=update_lijst)
filter_knop.grid(row=10, column=1, sticky="ew", padx=5)

# Knoppen om te exporteren naar Excel en SQL
exporteer_excel_knop = tk.Button(root, text="Exporteer naar Excel", command=exporteer_naar_excel)
exporteer_excel_knop.grid(row=11, column=0, sticky="ew", padx=5)

exporteer_sql_knop = tk.Button(root, text="Exporteer naar SQL", command=exporteer_naar_sql)
exporteer_sql_knop.grid(row=11, column=1, sticky="ew", padx=5)

# Voer de initialisatie van de UI uit en werk de lijst bij
update_lijst()

root.mainloop()

# Sluit de databaseverbinding wanneer de applicatie wordt afgesloten
conn.close()
