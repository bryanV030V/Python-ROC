...
def exporteer_naar_sql():
    with open('kledingwinkel_export.sql', 'w') as f:
        for line in conn.iterdump():
            f.write('%s\n' % line)
    messagebox.showinfo("Succes", "Database succesvol geëxporteerd naar kledingwinkel_export.sql!")

# Functie om merken van de website te scrapen en toe te voegen aan de lijst
def voeg_merken_toe():
    url = "https://www.brandsdistribution.com/en/designers"
    ...
    try:
        ...
        # Zoeken naar merknamen op de pagina
        merken = soup.find_all('h5', class_='title-designer')  # Aangepast voor de juiste klasse

        if not merken:
            messagebox.showerror("Fout", "Geen merken gevonden op de website.")
            return

        for merk in merken:
            merk_naam = merk.text.strip()

            # Voeg de merken toe aan de database
            c.execute("INSERT INTO kledingstukken (merk, type, maat, hoeveelheid) VALUES (?, ?, ?, ?)", (merk_naam, "N/A", "N/A", 1))
        
        conn.commit()
        update_lijst()
        messagebox.showinfo("Succes", "Merken succesvol toegevoegd aan de lijst!")

    except requests.exceptions.RequestException as e:
        messagebox.showerror("Fout", f"Er is een fout opgetreden bij het ophalen van gegevens: {e}")
...
tk.Label(root, text="Filter Merk").grid(row=6, column=0)
filter_merk_entry = tk.Entry(root)
filter_merk_entry.grid(row=6, column=1)

tk.Label(root, text="Filter Type").grid(row=7, column=0)
filter_type_entry = tk.Entry(root)
filter_type_entry.grid(row=7, column=1)

tk.Label(root, text="Filter Maat").grid(row=8, column=0)
filter_maat_entry = tk.Entry(root)
filter_maat_entry.grid(row=8, column=1)

# Zoekknop
zoek_knop = tk.Button(root, text="Zoek", command=zoek_kledingstukken)
...
exporteer_knop.grid(row=11, column=0, columnspan=2, pady=10)
...
exporteer_sql_knop.grid(row=12, column=0, columnspan=2, pady=10)

# Knop om merken van de website toe te voegen
scrape_knop = tk.Button(root, text="Voeg Merken Toe van Website", command=voeg_merken_toe)
scrape_knop.grid(row=13, column=0, columnspan=2, pady=10)
